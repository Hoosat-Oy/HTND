...
func Options() opt.Options {
	return opt.Options{
		...
		WriteBuffer:            64 * opt.MiB,          // Smaller buffer for smaller SSTs
		BlockCacheCapacity:     512 * opt.MiB,         // Keep for read performance
		OpenFilesCacheCapacity: 2048,                  // Reduce slightly to limit open files
		BlockRestartInterval:   8,                     // Keep for faster read scans
		CompactionTableSize:    8 * opt.MiB,           // Smaller tables for faster compactions
		CompactionTotalSize:    512 * opt.MiB,         // Smaller total size to trigger compactions sooner
	}
}

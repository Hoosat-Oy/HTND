...
const minChangeTarget = constants.SompiPerHoosat * 10
...
func (s *server) calculateFeeLimits(requestFeePolicy *pb.FeePolicy) (feeRate float64, maxFee uint64, err error) {
	...
	case nil:
		...
		maxFee = constants.SompiPerHoosat
	}
...
func (s *server) estimateFee(selectedUTXOs []*libhtnwallet.UTXO, feeRate float64, maxFee uint64, recipientValue uint64) (uint64, error) {
	...
	var mockPayments []*libhtnwallet.Payment
	if totalValue > recipientValue {
		mockPayments = []*libhtnwallet.Payment{
			{
				Address: fakeAddr,
	...
	mockTx, err := libhtnwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
...
func (s *server) estimateFeePerInput(feeRate float64) (uint64, error) {
	...
	mockTx, err := libhtnwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
		nil, []*libhtnwallet.UTXO{mockUTXO})
	if err != nil {
		return 0, err
	}
	...
	mockTxWithoutUTXO, err := libhtnwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
		s.keysFile.MinimumSignatures,
		nil, nil)
	if err != nil {
		return 0, err

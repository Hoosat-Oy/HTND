...
func (s *server) mergeTransaction(
	...
	return libhtnwallet.CreateUnsignedTransaction(s.keysFile.ExtendedPublicKeys,
...
func createTransactionWithJunkFieldsForMassCalculation(transaction *serialization.PartiallySignedTransaction, ecdsa bool, minimumSignatures uint32, txMassCalculator *txmass.Calculator) (*externalapi.DomainTransaction, error) {
	...
	return libhtnwallet.ExtractTransactionDeserialized(transaction, ecdsa)
}
...
func (s *server) moreUTXOsForMergeTransaction(alreadySelectedUTXOs []*libhtnwallet.UTXO, requiredAmount uint64, feeRate float64) (

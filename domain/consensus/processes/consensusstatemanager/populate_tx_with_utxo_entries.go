...
func (csm *consensusStateManager) populateTransactionWithUTXOEntriesFromVirtualOrDiff(stagingArea *model.StagingArea,
	...
	for i := 0; i < len(transaction.Inputs); i++ {
		...
		go func(index int) {
			...
			outpointsMu.Lock()
			hasUTXOEntry, err := csm.consensusStateStore.HasUTXOByOutpoint(csm.databaseContext, stagingArea, &transaction.Inputs[index].PreviousOutpoint)
			outpointsMu.Unlock()
			if err != nil {
				errMu.Lock()
				if firstErr == nil {
			...
			outpointsMu.Lock()
			utxoEntry, err := csm.consensusStateStore.UTXOByOutpoint(csm.databaseContext, stagingArea, &transaction.Inputs[index].PreviousOutpoint)
			outpointsMu.Unlock()
			if err != nil {
				errMu.Lock()
				if firstErr == nil {
